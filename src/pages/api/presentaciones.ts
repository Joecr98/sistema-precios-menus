  ...
  try {
    ...
    if (req.method === "GET") {
      try {
        ...
        await connection.end();
        return res.status(500).json({ message: "Error al obtener las presentaciones" });
      }
    } 
      ...
      try {
        ...
        if (error.code === 'ER_DUP_ENTRY') {
          return res.status(400).json({ message: "Ya existe una presentación con ese nombre" });
        }
        return res.status(500).json({ message: "Error al crear la presentación" });
      }
    }
